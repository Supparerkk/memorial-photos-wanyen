...
import { HardDrive, ExternalLink, Maximize2, ImageOff, Download, X, ChevronLeft, ChevronRight, ArrowDown } from 'lucide-react';

// --- Helper Function 1: ปรับขนาดภาพ (Optimization) ---
const getOptimizedUrl = (url, type = 'thumbnail') => {
    ...
    if (url.includes('images.unsplash.com')) {
        ...
        if (type === 'thumbnail') return `${baseUrl}?q=60&w=400&auto=format&fit=crop`;
        else return `${baseUrl}?q=90&w=1200&auto=format&fit=crop`;
    }

    // 2. กรณีรูปจาก GitHub (หรือแหล่งอื่นๆ) **ส่วนที่แก้ไขเพื่อความเร็ว**
    // ใช้บริการ wsrv.nl เพื่อย่อรูปให้เล็กลงก่อนส่งมาที่หน้าเว็บ
    if (url.includes('raw.githubusercontent.com') || url.startsWith('http')) {
        if (type === 'thumbnail') {
            // ย่อเหลือความกว้าง 400px, คุณภาพ 75%, แปลงเป็น WebP (ไฟล์เล็กโหลดไว)
            return `https://wsrv.nl/?url=${encodeURIComponent(url)}&w=400&q=75&output=webp&il`;
        } 
        // ถ้าเป็น type 'full' (ดูรูปใหญ่) เราจะส่ง URL เดิมเพื่อให้ได้ความคมชัดสูงสุด
        // และเพื่อให้ปุ่ม Download ได้ไฟล์ต้นฉบับ
        return url;
    }
...
// --- Helper Function 2: (แบบเดิม) ---
const generateSequence = (baseUrl, count, extension = 'jpg') => {
    const cleanBaseUrl = baseUrl.replace(/\/$/, '');
    return Array.from({ length: count }, (_, i) => `${cleanBaseUrl}/${i + 1}.${extension}`);
};

// --- Helper Function 3: สำหรับไฟล์จากกล้อง ---
const generateCameraSequence = (baseUrl, count, prefix = 'DSCF', startNum = 1, padding = 4, extension = 'jpg') => {
    const cleanBaseUrl = baseUrl.replace(/\/$/, '');
...
const ThaiTopBorder = ({ className }) => (
    <div className={`w-full overflow-hidden leading-[0] bg-[#1a1a1a] ${className}`}>
        <svg width="100%" height="60" viewBox="0 0 1200 60" preserveAspectRatio="xMidYMid slice" fill="currentColor" className="text-[#c5a059] opacity-80">
            <defs>
                <pattern id="lai-thai-pattern" x="0" y="0" width="100" height="60" patternUnits="userSpaceOnUse">
                   <path d="M50 30 L65 45 L50 60 L35 45 Z" fill="none" stroke="currentColor" strokeWidth="1" opacity="0.5"/>
                   <path d="M50 30 L65 15 L50 0 L35 15 Z" fill="none" stroke="currentColor" strokeWidth="1" opacity="0.5"/>
                   <path d="M20 30 Q 35 10, 50 30 Q 65 50, 50 30" fill="none" />
                   <path d="M0 50 Q 15 50, 25 40 Q 35 30, 50 30 Q 65 30, 75 40 Q 85 50, 100 50" fill="none" stroke="currentColor" strokeWidth="1.5" />
                   <path d="M0 10 Q 15 10, 25 20 Q 35 30, 50 30 Q 65 30, 75 20 Q 85 10, 100 10" fill="none" stroke="currentColor" strokeWidth="1.5" />
            ...
            <rect x="0" y="0" width="100%" height="5" fill="url(#gold-gradient)" opacity="0.7" />
            <rect x="0" y="55" width="100%" height="5" fill="url(#gold-gradient)" opacity="0.7" />
            <rect x="0" y="5" width="100%" height="50" fill="url(#lai-thai-pattern)" />
        </svg>
    </div>
);
...
const LotusIcon = ({ className }) => (
    <svg viewBox="0 0 100 100" className={className} fill="none" stroke="currentColor" strokeWidth="1.2">
        <path d="M50 10 C 50 10, 30 40, 30 60 C 30 80, 50 90, 50 90 C 50 90, 70 80, 70 60 C 70 40, 50 10, 50 10 Z" className="text-[#c5a059]" />
        <path d="M50 15 C 50 15, 40 45, 40 60 C 40 70, 50 80, 50 80 C 50 80, 60 70, 60 60 C 60 45, 50 15, 50 15 Z" fill="currentColor" opacity="0.3" />
...
const EVENT_DATA = {
    ...
    description: "ขอกราบขอบพระคุณแขกผู้มีเกียรติทุกท่าน ที่มาร่วมไว้อาลัยและส่งดวงวิญญาณสู่สุคติ\nในวาระสุดท้ายนี้ ความเมตตาของท่านจะสถิตในใจเราตลอดไป"
};
...
const DAYS_DATA = [
    {
        ...
        images: [
            // --- ดึงรูปจาก GitHub (วันที่ 25) ---
            ...generateCameraSequence(
                "https://raw.githubusercontent.com/Supparerkk/memorial-wanyen/main/day-25-wanyen", 
                50,     // จำนวนรูป
                "",     // Prefix
                1,      // StartNum
                3,      // Padding
    ...
    },
    {
        ...
        images: [
            // --- ดึงรูปจาก GitHub (วันที่ 26) ---
            ...generateCameraSequence(
                "https://raw.githubusercontent.com/Supparerkk/memorial-wanyen/main/day-26-wanyen", 
                100,    // จำนวนรูป
                "",     // Prefix
                1,      // StartNum
                3,      // Padding
    ...
    },
    {
        ...
        label: "วันที่ ๒๗",
        date: "พิธีฌาปนกิจศพ",
        driveLink: "https://drive.google.com/drive/folders/1Bpm6Z2DBZnt-ge2ugY3eLCQD4lWv803o",
        
        images: [
            // --- ดึงรูปจาก GitHub (วันที่ 27) ---
            ...generateCameraSequence(
                "https://raw.githubusercontent.com/Supparerkk/memorial-wanyen/main/day-27-wanyen", 
                109,    // จำนวนรูป
                "",     // Prefix
                1,      // StartNum
                3,      // Padding
...
export default function App() {
    ...
    const handlePrev = useCallback((e) => {
        e?.stopPropagation();
        if (selectedImageIndex === null) return;
        setSelectedImageIndex((prev) => (prev - 1 + activeData.images.length) % activeData.images.length);
    }, [selectedImageIndex, activeData.images.length]);
    ...
    const handleDownload = async (url) => {
        try {
            // สำหรับปุ่ม Download: ใช้ URL ต้นฉบับจาก GitHub โดยตรง (ไม่ผ่านการย่อ)
            // ฟังก์ชัน getOptimizedUrl ด้านบนถูกแก้ให้คืนค่า url เดิมถ้า type ไม่ใช่ 'thumbnail'
    ...
    return (
        <div className="min-h-screen pb-20 selection:bg-[#c5a059] selection:text-black bg-[#050505] text-[#e0e0e0] font-sans">
            {/* Inject Fonts & Global Styles - ใน React จริง ควรไปใส่ใน index.css หรือ import font ใน index.html */}
            <style>
                {`
                    /* Chonburi สำหรับหัวข้อใหญ่ สไตล์เรโทร/ขลัง */
                    @import url('https://fonts.googleapis.com/css2?family=Chonburi&display=swap');
                    /* Niramit สำหรับเนื้อหา สไตล์ทางการ มีหัว */
            ...
            {/* Navigation Bar */}
            <nav className={`fixed top-0 left-0 right-0 z-40 transition-all duration-700 ${isScrolled ? 'bg-[#050505]/95 backdrop-blur-md border-b border-[#c5a059]/30 py-4 mt-12' : 'bg-transparent py-8 mt-12'}`}>
                <div className="container mx-auto px-4 flex justify-center items-center">
                    <div className={`text-[#c5a059] font-medium text-xl tracking-wider thai-header transition-opacity duration-500 ${isScrolled ? 'opacity-100' : 'opacity-0'}`}>
                        {EVENT_DATA.title}
                    </div>
                </div>
            </nav>
            ...
            <header className="relative pt-44 pb-20 px-4 text-center">
                <div className="fade-in max-w-4xl mx-auto">
                    ...
                    <p className="text-[#c5a059] text-base md:text-xl mt-4 font-light tracking-wider break-words">
                        ...
                        <span className="text-sm md:text-base opacity-80 text-gray-400">{EVENT_DATA.location}</span>
                    </p>
            ...
            <main className="container mx-auto px-4 py-16 min-h-[60vh]">
                ...
                <div className="text-center mb-12 fade-in border-b border-[#c5a059]/20 pb-8">
                    ...
                    <p className="text-gray-500 text-sm mt-4 font-light tracking-wide">รวมภาพความทรงจำอันทรงคุณค่า</p>
                </div>
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 fade-in">
                            {activeData.images.map((img, idx) => (
                                <div 
                                    key={idx}
                                    className="relative aspect-[3/4] md:aspect-square group cursor-pointer overflow-hidden bg-[#151515] shadow-lg border border-[#333] hover:border-[#c5a059]/50 transition-all duration-500"
                                ...
                                >
                                    ...
                                    <img 
                                        ...
                                        loading="lazy"
                                        decoding="async"
                                        // ซ่อน parent div ทันทีถ้าโหลดรูปไม่ได้ (แก้ปัญหารูปข้ามเลข)
                                        onError={(e) => {
            ...
            {selectedImageIndex !== null && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/98 backdrop-blur-xl p-0 md:p-4 fade-in"
                    ...
                    <div className="absolute top-0 left-0 right-0 p-4 md:p-6 flex justify-between items-start z-50 bg-gradient-to-b from-black via-black/80 to-transparent h-32 pointer-events-none">
                        <div className="text-[#c5a059]/80 text-lg font-serif tracking-widest pointer-events-auto">
                            {selectedImageIndex + 1} <span className="text-xs text-gray-600 mx-2">/</span> {activeData.images.length}
                        </div>
                        <div className="flex space-x-4 pointer-events-auto">
                            <button 
                                ...
                                title="Download"
                            >
                                <span className="text-xs uppercase tracking-widest block opacity-100 text-[#c5a059]">บันทึกรูป</span>
                    ...
                    <div className="relative w-full h-full flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
                        <div className="relative border-8 border-white p-2 bg-white shadow-2xl max-h-[80vh] max-w-[90vw] rotate-1 transform transition-transform duration-500 hover:rotate-0">
                            <img 
                                src={getOptimizedUrl(activeData.images[selectedImageIndex], 'full')} 
                                alt="Full view" 
                        ...
                        {/* Mobile Tap Areas (ยังคงไว้สำหรับการแตะหน้าจอ) */}
                        <div className="absolute inset-y-0 left-0 w-1/5 z-10" onClick={handlePrev}></div>
                        <div className="absolute inset-y-0 right-0 w-1/5 z-10" onClick={handleNext}></div>
                    </div>
                </div>
            )}
        </div>
